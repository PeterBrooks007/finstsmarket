...
const DraftComponent = ({ setOpenEdit, toggleDrawer }) => {
      ...
      if (!isLoading && allMails.length !== 0) {
        const allMessages = allMails.reduce((accumulator, mailbox) => {
          ...
          const messagesWithUserId = mailbox.messages
            .filter((message) => message.folder === "draft")
            .map((message) => ({
              ...message,
      ...
      >
        ...
        >
          <Typography color={"grey"} variant="body2">
            DRAFT MESSAGES ({messages.length})
          </Typography>
        ...
        (isLoading ? (
          ...
          <Stack mt={2} spacing={0} display={{ xs: "flex", md: "none" }}>
            {filteredAllMailInbox && filteredAllMailInbox.length > 0 ? (
              ...
              <XCircle size={70} />
              <Typography variant="h5">No draft Messages</Typography>
            </Stack>
            )}
          </Stack>
        ))}
