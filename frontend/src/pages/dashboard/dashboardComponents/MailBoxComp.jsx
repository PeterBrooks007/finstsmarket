...
export default function CustomDrawer() {
  ...
  const unreadMessageCount = allMails.reduce((accumulator, mailbox) => {
    const messagesWithUserId = mailbox.messages
      .filter(message => message.from === "Support Team" && message.isRead === false )
      .map(message => ({
        ...message,
        userId: mailbox.userId, // Add userId to each message
      }));
    
    return accumulator.concat(messagesWithUserId).length;
  }, []);
