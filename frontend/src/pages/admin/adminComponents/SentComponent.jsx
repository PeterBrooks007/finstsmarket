...
const SentComponent = ({ toggleDrawer }) => {
  ...
  const {isSemiLoading, isLoading, allMailSent } = useSelector((state) => state.mailbox);

  const [selectedMails, setSelectedMails] = useState([]);

  useEffect(() => {
    if (allMailSent.length === 0) {
  ...
  useEffect(() => {
    if (allMailSent.length !== 0) {
      setAllMailSentList(allMailSent);
    }
  }, [allMailSent]);
...
<ViewMessage openEdit={openEdit} setOpenEdit={setOpenEdit} from={"sentComponent"} />
      ...
      >
        ...
        >
          <Typography color={"grey"} variant="body2">
            SENT MESSAGES ({allMailSent.length})
          </Typography>
        ...
        (isLoading ? (
          ...
          <Stack mt={2} spacing={3} display={{ xs: "flex", md: "none" }}>
            {filteredAllMailSent && filteredAllMailSent.length > 0 ? (
              ...
              <XCircle size={70} />
              <Typography variant="h5">No sent messages</Typography>
            </Stack>
            )}
          </Stack>
        ))}
