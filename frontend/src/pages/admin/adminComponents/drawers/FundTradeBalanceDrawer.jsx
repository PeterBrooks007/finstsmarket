...
const FundTradeBalanceDrawer = ({
  open,
  handleClose,
  handleOpen,
  fundTradeBalanceLoader,
  setFundTradeBalanceLoader,
}) => {
  ...
  useEffect(() => {
    if (fundTradeBalanceLoader) {
      const timer = setTimeout(() => {
        setFundTradeBalanceLoader(false);
      }, 300);
      return () => clearTimeout(timer);
    }
  }, [fundTradeBalanceLoader, setFundTradeBalanceLoader]);
  ...
  const saveProfile = async (values) => {
    ...
    await dispatch(adminFundTradeBalance({ id, userData }));
  };
...
export default FundTradeBalanceDrawer;
