...
import { updateTradingBot, updateTradingBotPhoto } from "../../../../redux/features/tradingBots/tradingBotsSlice";

const EditTradingBot = ({
  open,
  handleClose,
  handleOpen,
  tradingBotDrawerLoader,
  setTradingBotDrawerLoader,
}) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
  const dispatch = useDispatch();

  const { isLoading, tradingBot } = useSelector(
    (state) => state.tradingBots
  );

  useEffect(() => {
    if (tradingBotDrawerLoader) {
      const timer = setTimeout(() => {
        setTradingBotDrawerLoader(false);
      }, 300);
      return () => clearTimeout(timer);
    }
  }, [tradingBotDrawerLoader, setTradingBotDrawerLoader]);

  const [isEditing, setIsEditing] = useState(false);

  const initialState = {
    name: tradingBot?.name || "",
    price: tradingBot?.price || "",
    dailyTrades: tradingBot?.dailyTrades || "",
    winRate: tradingBot?.winRate || "",
    comment: tradingBot?.comment || "",
    photo: tradingBot?.photo || "",
  };
...
function decodeEntities(encodedString) {
  ...
  const userSchema = yup.object().shape({
    name: yup
      ...
      .min(2, "bot name must be at least 2 characters long")
  ...
  const handleFormSubmit = async (values) => {
    ...
    await dispatch(updateTradingBot({ id, userData }));
  ...
  const savePhoto = async (e) => {
    ...
    try {
      if (profileImage !== null) {
        ...
        dispatch(updateTradingBotPhoto({ id, formData }));
