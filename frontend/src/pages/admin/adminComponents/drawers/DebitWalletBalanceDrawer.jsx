...
import { adminDebitAssetBalance, adminDeleteAssetWalletFromUser, adminFundAssetBalance, adminSetIsManualAssetMode } from "../../../../redux/features/auth/authSlice";
...
const DebitWalletBalanceDrawer = ({
  open,
  handleClose,
  handleOpen,
  debitWalletBalanceLoader,
  setDebitWalletBalanceLoader,
}) => {
  ...
  useEffect(() => {
    if (debitWalletBalanceLoader) {
      const timer = setTimeout(() => {
        setDebitWalletBalanceLoader(false);
      }, 300);
      return () => clearTimeout(timer);
    }
  }, [debitWalletBalanceLoader, setDebitWalletBalanceLoader]);
  ...
  const fundwallet = () => {
    ...
    dispatch(adminDebitAssetBalance({ id, userData }));
      ...
      >
        {debitWalletBalanceLoader ? (
          ...
          >
            ...
            <Stack direction={"row"} spacing={0.5} alignItems={"flex-start"} border={"1px solid red"} p={0.5}>
              <Box>
              <XCircle size={42} color="red" />
              </Box>
              <Typography variant="subtitle2">
              Unable to get fiat rate balance for coins at this time, please kindly make calculations manually for now.
              </Typography>
            </Stack>
            </Box>
      ...
      >
        ...
        >
          <Stack spacing={1}>
            ...
            <Stack direction={"row"} alignItems={"center"} spacing={1}>
            <Typography>{!isCryptoInput ? "Fiat Method" : "Crypto Method"}</Typography>
            <IconButton size="small" sx={{border: "1px solid grey"}} onClick={handleCloseModal}>
            <X />
            </IconButton>
            </Stack>
            </Stack>
            ...
            <Typography>Price: {coinPriceLoading ? (
                        <Skeleton variant="text" width={"200px"} />
                      ) : conversionRate?.rate ? (
                        Intl.NumberFormat("en-US", {
                          style: "currency",
          ...
          <Stack my={3}>
            <Typography fontWeight={700} color={"springgreen"}>{selectedAsset?.name} Balance</Typography>
            <Typography > Fiat Balance: <span style={{color: "springgreen"}}>{
                Intl.NumberFormat("en-US", {
                  style: "currency",
                  currency: singleUser?.currency?.code,
            ...
            <Typography variant="subtitle2">{selectedAsset?.balance} {selectedAsset?.symbol.toUpperCase()} </Typography>
          </Stack>
          ...
          <Stack direction={"row"} spacing={1} alignItems={"center"}>
            <Stack spacing={0.5} width={"100%"}>
              ...
              <OutlinedInput
                ...
                type="text"
                name="amount"
                value={isCryptoInput ? amountInCryoto * selectedAsset?.price : amount}
            ...
            <Stack pt={4}>
              <IconButton sx={{border: "2px solid green"}} onClick={() => 
               { setAmount(0)
                setAmountInCryoto(0)
            ...
            >
              ...
              <OutlinedInput
                type="text"
                value={ isCryptoInput ? amountInCryoto : allCoins.length === 0 ? amountInCryoto : Number(amount / selectedAsset?.price).toFixed(8)}
                ...
                endAdornment={
                  <InputAdornment position="end">
                    <Typography>{selectedAsset?.symbol.toUpperCase()}</Typography>
                  </InputAdornment>
                }
              />
...
export default DebitWalletBalanceDrawer;
