...
import { getAllAdminNotifications, getUserNotifications } from "../../redux/features/notifications/notificationsSlice.js";
import AllAdminNotifications from "../AllAdminNotifications.jsx";
...
const AllAdminNotificationsDrawer = ({
  open,
  handleClose,
  handleOpen,
  ...
  useEffect(() => {
    if (allNotificationsLoader) {
      const timer = setTimeout(() => {
        setAllNotificationsLoader(false);
      }, 300);
      return () => clearTimeout(timer);
    }
  }, [allNotificationsLoader, setAllNotificationsLoader]);
      ...
      >
        {allNotificationsLoader || !user ? (
          ...
          >
            ...
            >
              <Stack spacing={1} p={"5px 15px 5px 15px"}>
                <Stack
                  direction={"row"}
                  justifyContent={"space-between"}
                  p={"5px 5px"}
                ...
                >
                  ...
                  <IconButton
                    size="small"
                    onClick={() => dispatch(getAllAdminNotifications())}
                  >
              ...
              <Box m={0} height={"100%"} overflow={"auto"}>
                <AllAdminNotifications allNotification={true} />
              </Box>
            </Stack>
          </Box>
        )}
...
export default AllAdminNotificationsDrawer;
